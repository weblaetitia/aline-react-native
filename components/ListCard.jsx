...
function ListCard(props) {
  ...
  const addFav = async (id) => {
    ...
    if (!props.token || props.token === '' || props.token === undefined) {
      ...
      const rawResponse = await fetch(`${BASE_URL}/users/mobile/${verb}?token=${props.token}&placeid=${id}`)
      ...
      >
        ...
        <View style={{ ...styles.myTitle, marginLeft: 10, marginRight: 10 }}>
          <View style={{ display: 'flex', flexDirection: 'row' }}>
            <Image style={{ width: 18, height: 18, marginTop: 3 }} source={type === 'shop' ? shopImage : restaurantImage} />
            <Text
              style={{
                ...styles.h3blue,
                fontSize: 20,
        ...
        <View style={{ width: 30, marginHorizontal: 5 }}>
          <TouchableOpacity onPress={() => addFav(_id)}>
            {liked ? <FontAwesome name="heart" size={24} color={tomato} /> : <FontAwesome name="heart-o" size={24} color={grayMedium} />}
          </TouchableOpacity>
        </View>
      </View>
