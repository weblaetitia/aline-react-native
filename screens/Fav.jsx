    ...
    <View style={{ ...styles.container }}>
      {status === 'nolog' && (
        <View style={{ paddingHorizontal: 40 }}>
          <Text style={{ ...styles.current16, textAlign: 'center' }}>Vous devez être connecté pour ajouter des lieux à vos favoris</Text>
      ...
      {status !== 'nolog' && status === 'favlist' && favs.length > 0 && (
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          {favs.map((fav) => (
            <TouchableOpacity key={fav._id} onPress={() => navigation.navigate('Place', { place: fav })}>
